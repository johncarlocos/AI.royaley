#!/usr/bin/env python3
"""
ROYALEY - Evaluation Feature Engineering Patch
===============================================
Fixes the critical mismatch: models were trained on 87-120+ engineered features
but evaluation only feeds 79 raw CSV columns.

This script patches evaluate_models.py to:
1. Add engineer_features() - replicates all 30+ derived features from training_service.py
2. Call it in load_validation_data() before feature column selection
3. Fix load_and_predict() to handle feature count mismatches gracefully
...
NEW_PREDICT_SKLEARN = '''def _predict_sklearn(model_path: str, model_dir: str,
    ...
    if train_features and len(train_features) == expected_n:
...
def apply_patches():
    ...
    if 'def engineer_features(' not in content:
        ...
        if marker in content:
            ...
            changes.append("Added engineer_features() function")
        else:
    ...
    if 'engineer_features(' not in content.split('def load_validation_data')[1].split('def ')[0] if 'def load_validation_data' in content else '':
    ...
    # --- Patch 3: Replace _predict_sklearn ---
    ...
    # --- Patch 4: Replace _predict_tensorflow ---
    ...
    # --- Patch 5: Replace _predict_autogluon ---
    ...
    # --- Patch 6: Fix exclude_cols to NOT exclude spread_close and total_close ---
...
if __name__ == "__main__":
    print("=" * 60)
    print("  ROYALEY Evaluation Feature Engineering Patch")
    print("=" * 60)
    apply_patches()